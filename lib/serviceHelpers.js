...
const { generateTextWithOpenRouter } = require('../services/ai/openrouter');
 ...
 * @param {object|string} options queue options or legacy system instruction string.
 * @returns {number|null} The user's position in the queue, or null if the queue is full.
 */
function enqueueConvRequest(message, options = {}) {
    ...
    const job = {
        ...
        contents: normalizedOptions.contents || null,
        systemInstruction: normalizedOptions.systemInstruction || 'you are a friendly discord bot.',
        skipReplySentinel: Boolean(normalizedOptions.skipReplySentinel),
        noReplySentinel: normalizedOptions.noReplySentinel || '[[OWGT_NO_REPLY]]',
        timestamp: Date.now(),
...
async function processConvQueue() {
    ...
    try {
        ...
        // Use the prompt payload captured when the job was queued.
        const promptContents = job.contents || job.cleanContent;
        const result = await callWithRetries(
            () => discordClient.ai.models.generateContent({ model: 'gemini-2.5-flash', contents: promptContents, config: { systemInstruction: job.systemInstruction } }),
        ...
        if (result.status === 'success') {
            ...
            if (job.skipReplySentinel && rawReply.toUpperCase() === job.noReplySentinel.toUpperCase()) {
                console.log(`[CONV_QUEUE][SKIP] Model returned ${job.noReplySentinel}; no queued reply sent.`);
                return;
            }

            await channel.send({ content: rawReply.toLowerCase(), reply: { messageReference: job.messageId } });
        } else {
            console.warn('[CONV_QUEUE] Gemini failed for queued reply. Trying OpenRouter next.');
            const openRouterResult = await generateTextWithOpenRouter(job.contents || [{ role: 'user', content: job.cleanContent }], {
                serviceKey: 'openrouter_chat',
                systemInstruction: job.systemInstruction,
                temperature: 0.7
            });

            if (openRouterResult.status === 'success') {
                const rawReply = (openRouterResult.data || '').trim();
                if (!rawReply) return;

                if (job.skipReplySentinel && rawReply.toUpperCase() === job.noReplySentinel.toUpperCase()) {
                    console.log(`[CONV_QUEUE][SKIP] OpenRouter returned ${job.noReplySentinel}; no queued reply sent.`);
                    return;
                }
...
function renderWelcomeTemplate(template, userStr, inviterStr, ccUserStr, pointsMsg) {
    if (!template) return '';
    return template
        .replaceAll('{user}', String(userStr))
        .replaceAll('{inviter}', String(inviterStr))
        .replaceAll('{cc}', String(ccUserStr))
        .replaceAll('{points_msg}', String(pointsMsg));
}
